...
data <- raw_data %>%
  ...
  mutate(
    topicConfident = format(round(Probability, 2), nsmall = 2),
    sentimentConfident = format(round(as.numeric(score), 2), nsmall = 2),
    .keep = "unused"
  ) %>%
...
```{r}
topic_by_month <- data %>%
  group_by(month, topicId) %>%
  count() %>%
  arrange(month, topicId) %>%
  rename(count = n)
